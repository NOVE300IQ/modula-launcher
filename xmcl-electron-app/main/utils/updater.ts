...
async function downloadAsarUpdate(
  ...
  const urls = gfw.inside
    ? [`https://github.com/NOVE300IQ/modula-launcher/releases/download/v${version}/${file}`]
    : [`https://github.com/NOVE300IQ/modula-launcher/releases/download/v${version}/${file}`]
...
async function hintUserDownload(): Promise<void> {
  shell.openExternal('https://discord.gg/gyjG6NjYcR')
}
...
async function downloadAppInstaller(
  ...
  const destination = join(app.getPath('downloads'), 'modula.appinstaller')
  const url = 'https://github.com/NOVE300IQ/modula-launcher/releases/latest/download/modula.appinstaller'
      ...
      Object.assign(executor, {
        ...
        createRequest: (options: any, callback: any) => {
          if (gfw.inside) {
            ...
            options.pathname = `/Soft_Mirrors/github-release/NOVE300IQ/modula-launcher/LatestRelease/${basename(options.pathname)}`
...
export class ElectronUpdater implements LauncherAppUpdater {
  ...
  async #getUpdateFromSelfHost(): Promise<ReleaseInfo> {
    ...
    const response = await this.app
      .fetch(`https://api.github.com/repos/NOVE300IQ/modula-launcher/releases/latest`, {
        headers: {
