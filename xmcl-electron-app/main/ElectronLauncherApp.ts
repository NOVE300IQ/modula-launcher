...
function getErrorCode(e: Error) {
  ...
  } else if (e.message.includes('SSLV3_ALERT_HANDSHAKE_FAILURE') || e.message.includes('ERR_SSL_PROTOCOL_ERROR')) {
    // Handle SSL handshake failures by allowing a retry with fallback
    return undefined 
  }
  return code
}
