...
export async function exposeLocalPort(priv: number, pub: number) {
  ...
  const mappings = [{
    description: `Modula Multiplayer - udp - ${priv} - ${pub}`,
    protocol: 'udp',
    private: priv,
    public: pub,
    ttl: 24 * 60 * 60,
  }, {
    description: `Modula Multiplayer - tcp - ${priv} - ${pub}`,
    protocol: 'tcp',
    private: priv,
    public: pub,
    ttl: 24 * 60 * 60,
  ...
  const existedMappings = currentMappings.filter(m => m.description.indexOf('Modula Multiplayer') !== -1 &&
    m.private.port === priv &&
    m.private.host === ip &&
    m.public.port === pub &&
    m.enabled)
