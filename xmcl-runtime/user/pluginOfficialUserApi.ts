...
export const pluginOfficialUserApi: LauncherAppPlugin = async (app) => {
  ...
  const system = new MicrosoftAccountSystem(logger,
    ...
    new MicrosoftOAuthClient(
      ...
      async (directRedirectToLauncher) => {
        ...
        return (directRedirectToLauncher ? `http://localhost:${port}/auth` : `https://modula.app/auth?port=${port}`)
      },
      (response) => {
  ...
  app.protocol.registerHandler('modula', ({ request, response }) => {
    ...
    } else if (parsed.host === 'launcher' && (parsed.pathname === '/favicon.ico' || parsed.pathname === '/logo.png')) {
      response.status = 200
      response.headers = {
        'Content-Type': 'image/png',
        'Cache-Control': 'public, max-age=3600',
      }
      // Use the 256px PNG for both logo and favicon to ensure Modula branding
      const imagePath = app.builtinAppManifest.iconSets.darkIcon || app.builtinAppManifest.iconSets.icon
      response.body = createReadStream(imagePath)
    }
  })
}
