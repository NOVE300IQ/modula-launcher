...
export class PresenceService extends AbstractService implements IPresenceService {
  ...
  constructor(@Inject(LauncherAppKey) app: LauncherApp,
    ...
    // Listen to game launch events to update presence when game is running
    launchService.on('minecraft-start', (event) => {
      this.runningGameCount++
      this.log(`Game started, updating Discord presence for Modula Minecraft (running games: ${this.runningGameCount})`)
      // Set playing activity when first game starts
      if (this.runningGameCount === 1 && this.discord.isConnected) {
        this.discord.user?.setActivity({
          details: `Playing Modula Minecraft`,
          state: `Version ${event.version}`,
          largeImageKey: 'dark_512',
          startTimestamp: Date.now(),
          buttons: [
            { label: 'Download Modula', url: 'https://www.modulamc.in/' }
          ]
        }).catch((e: any) => {
          this.warn('Fail to update discord presence. %o', e)
        })
      }
    })
