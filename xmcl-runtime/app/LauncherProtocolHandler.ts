 ...
 *
 * The drop in request or localhost request will be transformed into modula:// protocol
 */
  ...
  async handle(request: RequestOptions): Promise<ResolvedResponse> {
    ...
    const context: Context = {
      request: {
        ...
        url: typeof request.url === 'string' ? new URL(request.url, 'modula://launcher') : request.url,
