...
export class LauncherApp extends EventEmitter {
  ...
  get userAgent() {
    ...
    return `TeamPixelBuff/modula_launcher/${version}`
  }
  ...
  readonly server: Server = createServer((req, res) => {
    this.protocol.handle({
      method: req.method,
      url: new URL(req.url ?? '/', 'modula://launcher'),
      headers: req.headers,
      body: req,
    }).then((resp) => {
  ...
  protected async setup() {
    ...
    // register modula protocol
    if (!this.host.isDefaultProtocolClient('modula')) {
      const result = this.host.setAsDefaultProtocolClient('modula')
      if (result) {
        this.logger.log('Successfully register the modula protocol')
      } else {
        this.logger.log('Fail to register the modula protocol')
      }
    }
  ...
  protected async getStartupUrl(): Promise<string | undefined> {
    if (!IS_DEV && process.platform === 'win32') {
      ...
      if (process.argv.length > 1) {
        ...
        const protocolOption = process.argv.find(a => a.startsWith('modula://'))
        if (protocolOption) {
        ...
        this.logger.log('Didn\'t find modula:// protocol')
      }
    }
