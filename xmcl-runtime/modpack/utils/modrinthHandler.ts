  ...
  return {
    ...
    resolveInstanceFiles: (manifest: ModrinthModpackManifest): Promise<InstanceFile[]> => {
      const files = manifest.files
        .filter(meta => {
          // If env is not specified, assume it's required for both
          if (!meta.env) return true
          // If it's unsupported on the client, we skip it
          if (meta.env.client === 'unsupported') return false
          return true
        })
        .map(meta => ({
