  ...
  async function install(runtime: PartialRuntimeVersions) {
    ...
    if (metadata) {
      ...
      const exception = new AnyError(
        ...
        `Cannot find the minecraft version: "${minecraft}". Please select a valid Minecraft version in the settings.`,
        {},
        {
          minecraft,
        },
...
export function useInstanceVersionInstallInstruction(
  ...
  async function handleInstallInstruction(instruction: InstanceInstallInstruction) {
    ...
    if (!instruction.runtime || !instruction.runtime.minecraft) {
      // Attempt heuristic parsing for legacy/migrated instances lacking a parsed runtime
      instruction.runtime = instruction.runtime || {}
      const versionStr = instruction.version || ''
      const mcMatch = versionStr.match(/\b(1\.\d{1,2}(?:\.\d{1,2})?)\b/)
      if (mcMatch) instruction.runtime.minecraft = mcMatch[1]
      
      const loaderFabric = versionStr.match(/fabric(?:-loader)?-([\d\.]+)/i)
      if (loaderFabric) instruction.runtime.fabricLoader = loaderFabric[1]
      
      const loaderForge = versionStr.match(/forge-([\d\.]+)/i)
      if (loaderForge && !versionStr.toLowerCase().includes('neoforge')) instruction.runtime.forge = loaderForge[1]
      
      const loaderNeoForge = versionStr.match(/neoforge-([\d\.]+)/i)
      if (loaderNeoForge) instruction.runtime.neoForged = loaderNeoForge[1]
      
      const loaderQuilt = versionStr.match(/quilt(?:-loader)?-([\d\.]+)/i)
      if (loaderQuilt) instruction.runtime.quiltLoader = loaderQuilt[1]
      
      const loaderOptifine = versionStr.match(/optifine_.*?_([a-zA-Z0-9_]+)/i)
      if (loaderOptifine) instruction.runtime.optifine = loaderOptifine[1]

      if (instruction.runtime.minecraft) {
        // Commit the auto-fixed runtime instantly so UI reflects the correct dependencies
        await editInstance({
          instancePath: instruction.instance,
    ...
    try {
      ...
      notify({
        ...
        body: err.message || 'The launcher could not automatically install this migrated version. Please open Instance Settings (the gear icon) and explicitly re-select your Minecraft and Mod Loader versions.',
        level: 'error',
      })
    }
  }
