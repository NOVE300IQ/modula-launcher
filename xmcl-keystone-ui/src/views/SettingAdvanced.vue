...
/**
 * Exports all settings from localStorage into a downloadable .modulasettings file.
 */
async function handleExportSettings() {
  try {
    ...
    a.href = url
    a.download = `modula-settings-${new Date().toISOString().split('T')[0]}.modulasettings`
...
/**
 * Prompts the user to select a .modulasettings file and imports the settings from it.
 */
async function handleImportSettings() {
  try {
    const input = document.createElement('input')
    input.type = 'file'
    input.accept = '.modulasettings'
