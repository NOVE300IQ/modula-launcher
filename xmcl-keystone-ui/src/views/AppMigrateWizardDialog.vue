            ...
            <template v-if="manifest">
              ...
              >
                ...
                <InstanceItem
                  ...
                  :runtime="m.options.runtime ?? { minecraft: '', forge: '', fabricLoader: '', quiltLoader: '', neoForged: '', optifine: '', labyMod: '' }"
                  :name="m.options.name"
...
const onSelectType = async (type: InstanceType) => {
  const defaultPath = type === 'modrinth'
    ...
    : type === 'tlauncher' as any
    ? await getGameDefaultPath('tlauncher' as any)
    : type === 'vanilla'
  ...
  // Ensure we check all folders by default to prevent "Install" errors
